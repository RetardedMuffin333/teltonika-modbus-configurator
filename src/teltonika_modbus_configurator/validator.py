...
REQUEST_DATA_TYPES = {
    "int8", "uint8", "int16", "uint16", "ascii", "hex", "bool", "pdu", "raw"
}
TCP_DATA_TYPES = {
    "binary", "string", "bool", "int8", "uint8", "int16", "uint16",
...
def validate_project(project: Project) -> list[ValidationMessage]:
    ...
    for device in project.devices:
        ...
        if len(request_names) != len(set(request_names)):
            messages.append(ValidationMessage("error", f"{device.name}: duplicate request names"))
    ...
    for mapping in project.mappings:
        ...
        if device is None:
            messages.append(ValidationMessage("error", f"{mapping.name}: unknown device '{mapping.device}'")); continue
        request = next((r for r in device.requests if r.name == mapping.request), None)
        if request is None:
            messages.append(ValidationMessage("error", f"{mapping.name}: unknown request '{mapping.request}' on {mapping.device}")); continue
        if mapping.permissions not in MAPPING_PERMISSIONS:
            messages.append(ValidationMessage("error", f"{mapping.name}: permissions must be r, w or rw"))
        ...
        if request.function.is_read and mapping.permissions == "w":
            messages.append(ValidationMessage("error", f"{mapping.name}: read request cannot be exposed Write-Only"))
        if request.function.is_write and mapping.permissions == "r":
            messages.append(ValidationMessage("error", f"{mapping.name}: write request cannot be exposed Read-Only"))

        end = mapping.register + mapping.count - 1
        if not TELTONIKA_TCP_REGISTER_MIN <= mapping.register <= TELTONIKA_TCP_REGISTER_MAX or end > TELTONIKA_TCP_REGISTER_MAX:
