...
def _expand_groups(data: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for group in data.get("device_groups", []) or []:
        ...
        for ordinal in range(count):
            ...
            device_name = _format_name(name_pattern, device="", index=index, ordinal=ordinal)
            devices.append({
                "name": device_name,
            ...
            for mapping_template in template.get("mappings", []) or []:
                ...
                mappings.append({
                    ...
                    "register": int(mapping_template["start_register"]) + ordinal * int(mapping_template.get("step", 1)),
                    "enabled": mapping_template.get("enabled", group.get("mapping_enabled", True)),
                    "permissions": mapping_template.get("permissions", "r"),
                    "data_type": mapping_template.get("data_type", "int16"),
                    "count": mapping_template.get("count", 1),
                })
...
def load_project(path: str | Path) -> Project:
    ...
    for item in data.get("connections", []):
        ...
        connections.append(SerialConnection(
            name=item["name"], device=item.get("device", "/dev/rs485"),
            baudrate=int(item.get("baudrate", 19200)), databits=int(item.get("databits", 8)),
            parity=str(item.get("parity", "none")), stopbits=int(item.get("stopbits", 2)),
        ))
    ...
    for item in raw_devices:
        devices.append(Device(
            name=item["name"], slave_id=int(item["slave_id"]), connection=item["connection"],
            period=int(item.get("period", 10)), timeout=int(item.get("timeout", 1)),
            enabled=_bool(item.get("enabled"), True),
            requests=[_request(req) for req in item.get("requests", [])],
        ))

    mappings = [ServerMapping(
        name=item["name"], device=item["device"], request=item["request"],
        register=int(item["register"]), register_type=str(item["register_type"]),
        enabled=_bool(item.get("enabled"), True), permissions=str(item.get("permissions", "r")),
        data_type=str(item.get("data_type", "int16")), count=int(item.get("count", 1)),
    ) for item in raw_mappings]

    tcp = data.get("tcp_server", {}) or {}
    tcp_server = TcpServerSettings(
        port=int(tcp.get("port", 502)), device_id=int(tcp.get("device_id", 101)),
    ...
    source_uci = None
    if source:
        source_uci = ImportedUciState(modbus_client=str(source["modbus_client"]), modbus_server=str(source["modbus_server"]))
