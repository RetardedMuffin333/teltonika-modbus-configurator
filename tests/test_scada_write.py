...
def test_create_write_target_is_disabled_fc06_and_uses_20000_write_block():
    ...
    assert target.mapping.register == 20000
...
def test_generated_uci_uses_disabled_fc06_and_20000_write_only_tag():
    project = _project()
    ...
    assert "option modbus_reg_num '20000'" in uci.modbus_server


def test_carel_coil_feedback_creates_disabled_fc05_at_20000():
    project = Project(
...
def test_explicit_write_start_cannot_move_write_mapping_below_20000():
    project = _project()
    target = create_scada_write_target(project, device_name="RDF_Test", read_request_name="CMD_Oper_Mode", write_block_start=1487)
    assert target.mapping.register == 20000


def test_bulk_layout_keeps_write_block_at_20000_or_above():
    project = _project()
    target = create_scada_write_target(project, device_name="RDF_Test", read_request_name="CMD_Oper_Mode")
    ...
    assert layout["CMD_Oper_Mode_w"][0] >= 20000
